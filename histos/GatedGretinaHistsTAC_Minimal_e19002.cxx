...
bool HandleS800_Gated(TRuntimeObjects &obj,TCutG *incoming, TCutG* outgoing, TCutG* xToF) {
  ...
  if(incoming) {  
    ...
    for(int x=0;x<s800->GetMTof().ObjSize();x++) {
      for(int y=0;y<s800->GetMTof().XfpSize();y++) {
  ...
  ///////////////////////////////////////////////////////////////////
  if(outgoing) {
    if(!outgoing->IsInside(s800->GetCorrTOF_XFPTAC(),s800->GetIonChamber().Charge()))
      {return false;}
...
//Histogram ToF vs XTof
    ...
    for(int y=0;y<s800->GetMTof().XfpSize();y++) {
      if(s800->GetMTof().ObjSize() && s800->GetMTof().XfpSize()) {
        obj.FillHistogram(dirname,Form("Xfp-E1_v_Obj-E1_Uncorrected%s",hsuf.c_str()),
		    6000,8000,14000,s800->GetMTof().fObj[x],
  ...
  obj.FillHistogram(dirname,Form("Xfp-E1_v_Obj-E1_Uncorrected%s",hsuf.c_str()),
		6000,8000,14000,s800->GetMTof().GetCorrelatedObjE1(),
		8000,12000,20000,s800->GetMTof().GetCorrelatedXfpE1());

  obj.FillHistogram(dirname,Form("Charge_v_Obj-E1_Uncorrected%s",hsuf.c_str()),
		6000,8000,14000,s800->GetMTof().GetCorrelatedObjE1(),
		10000,0,30000,s800->GetIonChamber().Charge());
...
//  obj.FillHistogram(dirname,Form("Gamma_Raw%s",hsuf.c_str()),500,0,4000,hit.GetCoreEnergy());
  obj.FillHistogram(dirname,Form("Gamma(Beta)%s",hsuf.c_str()),500,0,4000,hit.GetDoppler(beta));
  ...
  obj.FillHistogram(dirname,Form("Gamma(Beta&Track)%s",hsuf.c_str()),500,0,4000,hit.GetDoppler(beta,&track));
  ...
  obj.FillHistogram(dirname,Form("Gamma(Beta&Track&YTA)%s",hsuf.c_str()),500,0,4000,hit.GetDopplerYta(beta,yta,&track));
...
//This section of code is all for track-reconstructed corrections, so it is commented out until I figure out how to use that.
  ...
  obj.FillHistogram(dirname,Form("CoreEn_v_TSDiff%s",hsuf.c_str()),
		    ...
		    2000,0,8000,hit.GetCoreEnergy());
*/
  obj.FillHistogram(dirname,Form("FineGamma(Beta)_v_DetMap%s",hsuf.c_str()),
  ...
  if(quadMap[hit.GetHoleNumber()] <= 4) {
    ...
    obj.FillHistogram(dirname,Form("Gamma(Beta)_Backward%s",hsuf.c_str()),
      1000,0,4000,hit.GetDoppler(beta));
  }
...
//This line ensures that we are only executing this if we have a gate for each field (currently incoming and outgoing)
  if(!s800 || !gretina || !bank29 || !outgoing || !incoming)
    {return false;}
...
//IsInside tests whether or not the event is inside the gate
  if(incoming) {  
    if(!incoming->IsInside(s800->GetOBJRaw_TAC(),s800->GetXFRaw_TAC()))
      {return false;}
...
//There can be multiple detections associated with a single event, so we will loop over those here
  ...
  for(unsigned int i=0;i<gretina->Size();i++) {
    ...
    //////////////////////////////////////////////////////////////////////////////
    if(time_energy) {
      if(!time_energy->IsInside(bank29->Timestamp() - hit.Timestamp(),hit.GetCoreEnergy()))
        {continue;}  
    }
    size++;
