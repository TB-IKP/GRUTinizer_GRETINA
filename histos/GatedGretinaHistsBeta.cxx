...
//                  1250,-500,2000,s800->GetXFP());
  ...
  obj.FillHistogram(dirname,Form("CRDC1Y%s",hsuf.c_str()),300,-1000,1000,s800->GetYFP());
...
//This section of code is all for track-reconstructed corrections, so it is commented out until I figure out how to use that.
  ...
  obj.FillHistogram(dirname,Form("CoreEn_v_TSDiff%s",hsuf.c_str()),
		    ...
		    2000,0,8000,hit.GetCoreEnergy());
*/
  obj.FillHistogram(dirname,Form("FineGamma(Beta)_v_QuadMap%s",hsuf.c_str()),
...
//  double yta = s800->GetYta();
  ...
  obj.FillHistogram(dirname,Form("GammaGamma(Beta)%s",hsuf.c_str()),
                    2500,0,2500,hit1.GetDoppler(beta),
                    2500,0,2500,hit2.GetDoppler(beta));
/*
