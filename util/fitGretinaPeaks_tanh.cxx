...
const int REBIN_FACTOR = 5; //8000 and 10000 must be evenly divisible by this number
...
void fitGretinaPeaks(std::string data_file_name, std::string output_fn, std::string peak_input, std::string bg_line_input, 
  ...
  GH1D* data_hist((GH1D*)data_file->Get("Gretina_MesyIncoming_timeEnergy/Gamma(Beta&Track&YTA&DTA)_p42"));
  //GH1D* data_hist((GH1D*)data_file->Get("Gretina_incoming_timeEnergy/Gamma(Beta&Track&YTA&DTA)_p38"));
  ...
  for(unsigned int i=0;i<energies.size();i++) {
	...
	TH1* hw_com;
	  ...
	  if(peaks[energies.at(i)] && comps[energies.at(i)]) {
            hw = (GH1D*)fw->Get("gretina_B&T&Y&D");
          }
	  else if(peaks[energies.at(i)] && !comps[energies.at(i)]) {
	    hw = (GH1D*)fw->Get("gretina_B&T&Y&D");
          }
	  ...
	  hw_fep = (GH1D*)(((TH1*)fw->Get("gretina_B&T&Y&D"))->Clone());
	  hw_com = (GH1D*)(((TH1*)fw->Get("gretina_B&T&Y&D"))->Clone());
        }
        else {
	  if(peaks[energies.at(i)] && comps[energies.at(i)]) {
