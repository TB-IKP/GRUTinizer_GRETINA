...
double getEff(double energy) {
  ...
  return 4.532*pow(energy+100.,-0.621)*10.75/8.;
}
...
TF1 *constructBackground(std::string param_list) {
  ...
  if(t->GetSelectedRows() == 3) {
  ...
  else if(t->GetSelectedRows() == 5) {
    
    bg = new TF1("Double_Exp","[0]*TMath::Exp([1]*x)+[2]*TMath::Exp([3]*x)",0,10000);
    ...
    std::cout << "Background model TNtuple does not have 3 or 5 rows. "
...
void fitGretinaPeaks(std::string data_file_name, std::string output_fn, std::string peak_input, std::string bg_line_input, 
  ...
  GH1D* data_hist((GH1D*)data_file->Get("Gretina_incoming_timeEnergy/Gamma(Beta&AdjustedTrack&YTA&DTA)_p38"));
  ...
  for(unsigned int i=0;i<energies.size();i++) {
	...
	TH1* hw_fep = (GH1D*)(((TH1*)fw->Get("gretsim/gretina_B&T_fep"))->Clone());
	TH1* hw_com = (GH1D*)(((TH1*)fw->Get("gretsim/gretina_B&T_bg"))->Clone());
	hw->Sumw2();
  ...
  TF1* bg;
  if(npars == 2) {
    bg = new TF1("exp_bg","[0]*TMath::Exp([1]*x)",0,10000);
    ...
    bg = new TF1("exp_bg","[0]*TMath::Exp([1]*x)+[2]*TMath::Exp([3]*x)",0,10000);
    bg->SetParameter(0,fSum.GetFunc()->GetParameter(energies.size()+offset));
    bg->SetParameter(1,fSum.GetFunc()->GetParameter(energies.size()+offset+1));
