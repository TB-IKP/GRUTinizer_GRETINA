...
void fitGretinaPeaks(std::string data_file_name, std::string output_fn, std::string peak_input, std::string bg_line_input, 
  ...
  //GH1D* data_hist((GH1D*)data_file->Get("Gretina_MesyIncoming_timeEnergy/Gamma(Beta)_90_p41"));
  ...
  for(unsigned int i=0;i<energies.size();i++) {
	...
	TH1* hw_com;
	  ...
	  if(peaks[energies.at(i)] && comps[energies.at(i)]) {
            hw = (GH1D*)fw->Get("gretina_B&T_Fwd");
          }
	  else if(peaks[energies.at(i)] && !comps[energies.at(i)]) {
	    hw = (GH1D*)fw->Get("gretina_B&T_Fwd");
          }
	  ...
	  hw_fep = (GH1D*)(((TH1*)fw->Get("gretina_B&T_Fwd"))->Clone());
	  hw_com = (GH1D*)(((TH1*)fw->Get("gretina_B&T_Fwd"))->Clone());
        }
        else {
	  if(peaks[energies.at(i)] && comps[energies.at(i)]) {
            hw = (GH1D*)fw->Get("gretsim/gretina_B&T_Fwd");
          }
	  else if(peaks[energies.at(i)] && !comps[energies.at(i)]) {
	    hw = (GH1D*)fw->Get("gretsim/gretina_B&T_fep_Fwd");
          }
	  else if(!peaks[energies.at(i)] && comps[energies.at(i)]) {
	    hw = (GH1D*)fw->Get("gretsim/gretina_B&T_bg_Fwd");
          }

	  hw_fep = (GH1D*)(((TH1*)fw->Get("gretsim/gretina_B&T_fep_Fwd"))->Clone());
	  hw_com = (GH1D*)(((TH1*)fw->Get("gretsim/gretina_B&T_bg_Fwd"))->Clone());
        }
	hw->Sumw2();
  ...
  for(unsigned int i=0;i<bg_energies.size();i++) {
    ...
    if(!f->IsZombie()) {
      bg_hists.push_back((GH1D*)f->Get("gretsim/gretina_B&T_Fwd"));
      bg_hists.back()->Sumw2();
