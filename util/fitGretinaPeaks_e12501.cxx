...
const int REBIN_FACTOR = 10; //8000 and 10000 must be evenly divisible by this number

double getEff(double energy) {
  //return 4.532*pow(energy+100.,-0.621)*8.75/8.;
  return (4.532*pow(energy+100.,-0.621)*7/8.)*(1+TMath::TanH((energy-185.1)/82.5))/2;
}
...
void fitGretinaPeaks(std::string data_file_name, std::string output_fn, std::string peak_input, std::string bg_line_input, 
  ...
  GH1D* data_hist((GH1D*)data_file->Get("Gretina_MesyIncoming_timeEnergy/Gamma(Beta&Track&YTA&DTA)_p40"));
  //GH1D* data_hist((GH1D*)data_file->Get("Gretina_Ungated/GretinaCoreEnergy"));
