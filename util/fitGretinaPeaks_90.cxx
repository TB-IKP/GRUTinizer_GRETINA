...
void fitGretinaPeaks(std::string data_file_name, std::string output_fn, std::string peak_input, std::string bg_line_input, 
  ...
  //GH1D* data_hist((GH1D*)data_file->Get("Gretina_MesyIncoming_timeEnergy/Gamma(Beta&AdjustedTrack&YTA&DTA)_p41"));
  //GH1D* data_hist((GH1D*)data_file->Get("Gretina_MesyIncoming_timeEnergy/Gamma(Beta)_Forward_p41"));
  GH1D* data_hist((GH1D*)data_file->Get("Gretina_incoming_timeEnergy/Gamma(Beta)_90_p38"));
  //GH1D* data_hist((GH1D*)data_file->Get("Gretina/Gamma(Beta&AdjustedTrack&YTA&DTA)_p43_p43_xfp"));
  //GH1D* data_hist((GH1D*)data_file->Get("Gretina_incoming_timeEnergy/Gamma(Beta&AdjustedTrack&YTA&DTA)_p39"));
  //GH1D* data_hist((GH1D*)data_file->Get("Gretina_Ungated/GretinaCoreEnergy_NoThresh"));
  //GH1D* data_hist((GH1D*)data_file->Get("Gretina_Ungated/CoreEnergy_det43"));
  //GH1D* data_hist((GH1D*)data_file->Get("Gretina_Inc_TE/Gamma(Beta&Track)_ZOut_xXfp"));
  //GH1D* data_hist((GH1D*)data_file->Get("GretinaDetMap_Inc_TE/Gamma(NewBeta&Track)_Fwd_ZOut_xToF"));
  //GH1D* data_hist((GH1D*)data_file->Get("GretinaDetMap_Inc_TE/Gamma(NewBeta&Track)_90Deg_ZOut_xToF"));
  if(!data_hist) {
  ...
  for(unsigned int i=0;i<energies.size();i++) {
    ...
    if(!f->IsZombie()) {
      if(energies.at(i) == 1) {
        if(peaks[energies.at(i)] && comps[energies.at(i)]) {
          fit_hists.push_back((GH1D*)f->Get("gretina_B&T&Y&D"));
        }
	  ...
	  fit_hists.push_back((GH1D*)f->Get("gretina_B&T&Y&D"));
        ...
        fep_hists.push_back((GH1D*)(((TH1*)f->Get("gretina_B&T&Y&D"))->Clone()));
        com_hists.push_back((GH1D*)(((TH1*)f->Get("gretina_B&T&Y&D"))->Clone()));
      }
      else {
        if(peaks[energies.at(i)] && comps[energies.at(i)]) {
          fit_hists.push_back((GH1D*)f->Get("gretsim/gretina_B&T_90Deg"));
	//fit_hists.push_back((GH1D*)f->Get("gretsim/gretina_B&T_Fwd"));
	//fit_hists.push_back((GH1D*)f->Get("gretsim/gretina_B&T_90Deg"));
        }
        else if(peaks[energies.at(i)] && !comps[energies.at(i)]) {
	  fit_hists.push_back((GH1D*)f->Get("gretsim/gretina_B&T_fep_90Deg"));
	//fit_hists.push_back((GH1D*)f->Get("gretsim/gretina_B&T_fep_Fwd"));
	//fit_hists.push_back((GH1D*)f->Get("gretsim/gretina_B&T_fep_90Deg"));
        }
        else if(!peaks[energies.at(i)] && comps[energies.at(i)]) {
	  fit_hists.push_back((GH1D*)f->Get("gretsim/gretina_B&T_bg_90Deg"));
	//fit_hists.push_back((GH1D*)f->Get("gretsim/gretina_B&T_bg_Fwd"));
	//fit_hists.push_back((GH1D*)f->Get("gretsim/gretina_B&T_bg_90Deg"));
        }
        fep_hists.push_back((GH1D*)(((TH1*)f->Get("gretsim/gretina_B&T_fep_90Deg"))->Clone()));
        com_hists.push_back((GH1D*)(((TH1*)f->Get("gretsim/gretina_B&T_bg_90Deg"))->Clone()));
      }

      //fep_hists.push_back((GH1D*)(((TH1*)f->Get("gretsim/gretina_B&T_fep_Fwd"))->Clone()));
      //com_hists.push_back((GH1D*)(((TH1*)f->Get("gretsim/gretina_B&T_bg_Fwd"))->Clone()));
      //fep_hists.push_back((GH1D*)(((TH1*)f->Get("gretsim/gretina_B&T_fep_90Deg"))->Clone()));
      //com_hists.push_back((GH1D*)(((TH1*)f->Get("gretsim/gretina_B&T_bg_90Deg"))->Clone()));
	...
	TH1* hw_com;
	  ...
	  if(peaks[energies.at(i)] && comps[energies.at(i)]) {
            hw = (GH1D*)fw->Get("gretina_B&T_90Deg");
          }
	  else if(peaks[energies.at(i)] && !comps[energies.at(i)]) {
	    hw = (GH1D*)fw->Get("gretina_B&T_90Deg");
          }
	  ...
	  hw_fep = (GH1D*)(((TH1*)fw->Get("gretina_B&T_90Deg"))->Clone());
	  hw_com = (GH1D*)(((TH1*)fw->Get("gretina_B&T_90Deg"))->Clone());
        }
        else {
	  if(peaks[energies.at(i)] && comps[energies.at(i)]) {
            hw = (GH1D*)fw->Get("gretsim/gretina_B&T_90Deg");
          }
	  else if(peaks[energies.at(i)] && !comps[energies.at(i)]) {
	    hw = (GH1D*)fw->Get("gretsim/gretina_B&T_fep_90Deg");
          }
	  else if(!peaks[energies.at(i)] && comps[energies.at(i)]) {
	    hw = (GH1D*)fw->Get("gretsim/gretina_B&T_bg_90Deg");
          }

	  hw_fep = (GH1D*)(((TH1*)fw->Get("gretsim/gretina_B&T_fep_90Deg"))->Clone());
	  hw_com = (GH1D*)(((TH1*)fw->Get("gretsim/gretina_B&T_bg_90Deg"))->Clone());
        }
	hw->Sumw2();
	...
	std::cout << "The histogram file for the wide component of the " << energies.at(i)
      ...
      /*
      if(energies.at(i)==447) {
	
	TFile* ft = new TFile("histFiles/hist0532_tau325.root","read");
	
	GH1D* fitt = (GH1D*)ft->Get("gretsim/gretina_B&T");
	GH1D* fept = (GH1D*)(((TH1*)ft->Get("gretsim/gretina_B&T_fep"))->Clone());
	GH1D* comt = (GH1D*)(((TH1*)ft->Get("gretsim/gretina_B&T_bg"))->Clone());
	
	fitt->GetXaxis()->SetLimits(fitt->GetXaxis()->GetXmin()+shifts[532],
				    fitt->GetXaxis()->GetXmax()+shifts[532]
				   );
	
	fept->GetXaxis()->SetLimits(fept->GetXaxis()->GetXmin()+shifts[532],
				    fept->GetXaxis()->GetXmax()+shifts[532]
				   );
	
	comt->GetXaxis()->SetLimits(comt->GetXaxis()->GetXmin()+shifts[532],
				    comt->GetXaxis()->GetXmax()+shifts[532]
				   );

	TList* fit_list = new TList;
	TList* fep_list = new TList;
	TList* com_list = new TList;
	
	fit_list->Add(fit_hists.back());
	fit_list->Add(fitt);
	fep_list->Add(fep_hists.back());
	fep_list->Add(fept);
	com_list->Add(com_hists.back());
	com_list->Add(comt);

	TH1D* hfit = (TH1D*)fit_hists.back()->Clone();
	TH1D* hfep = (TH1D*)fep_hists.back()->Clone();
	TH1D* hcom = (TH1D*)com_hists.back()->Clone();
	...
	fit_hists.push_back((GH1D*)hfit);
	fep_hists.push_back((GH1D*)hfep);
	com_hists.push_back((GH1D*)hcom);
	
      }
      */
      
      if(energies.at(i) == 1) {
        fit_hists.back()->Rebin(REBIN_FACTOR/2);
        fep_hists.back()->Rebin(REBIN_FACTOR/2);
        com_hists.back()->Rebin(REBIN_FACTOR/2); 
      }
      else {
  ...
  for(unsigned int i=0;i<bg_energies.size();i++) {
    ...
    if(!f->IsZombie()) {
      bg_hists.push_back((GH1D*)f->Get("gretsim/gretina_B&T_90Deg"));
      bg_hists.back()->Sumw2();
